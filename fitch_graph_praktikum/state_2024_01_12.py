...
for i, (mu_TP, mu_FP) in enumerate(tuples):
    ...
    benchmark_df, weight_df = benchmark_all_stored_graphs(mu_TP=mu_TP, mu_FP=mu_FP)

    save_dataframe(f'bm2_{mu_TP}_{mu_FP}_results', benchmark_df)
    save_dataframe(f'bm2_{mu_TP}_{mu_FP}_weights', benchmark_df)
